...
class CounterAState extends Equatable {
  // Equatable is use to tag state changing
  ...
  CounterAState copyWrite({int? count}) {
    //  Optional
    return CounterAState(
        count: count ??
            this.count); // but if count value was not send, so count = this.count (old state)
  }
  ...
  List<Object> get props => [
        count
      ]; // use Equatable by add count, if count value is changed. the value will show on interface. if you are not show. please empty []
}
